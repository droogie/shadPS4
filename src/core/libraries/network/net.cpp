...
int PS4_SYSV_ABI sceNetEpollWait(OrbisNetId epollid, OrbisNetEpollEvent* events, int maxevents,
    ...
    // P2P epoll fix: P2P sockets may share a native fd (VP40) or have separate
    // fds (VP30 on transport 1). Drain ALL transports and generate events for
    // ALL registered sockets that have queued data.
    if (is_p2p_epoll && result >= 0) {
        DrainP2PTransport();

        static std::atomic<int> s_p2p_fix_calls{0};
        int fix_call = s_p2p_fix_calls.load(std::memory_order_relaxed);
        ...
        for (const auto& [orbis_fd, ep_event] : epoll->events) {
            ...
            if (!file || file->type.load() != Core::FileSys::FileType::Socket)
                continue;
            bool has_data = file->socket && file->socket->HasQueuedData();
            // Log every socket check for first few calls, then only when data found
            if (has_data || fix_call < 5) {
                auto native = file->socket->Native();
                LOG_ERROR(Lib_Net,
                          "P2P DIAG: epoll fix check -- orbis_fd={} "
                          "native_fd={} has_data={} guest='{}' result={} "
                          "registered_sockets={} fix_call#={}",
                          orbis_fd, native ? *native : -1, has_data, file->m_guest_name, result,
                          epoll->events.size(), fix_call);
            }
            if (has_data) {
                events[p2p_count] = {
        ...
        {
            int c = s_p2p_fix_calls.fetch_add(1, std::memory_order_relaxed);
            if (c < 5) {
                LOG_ERROR(Lib_Net,
                          "P2P DIAG: epoll fix done -- p2p_count={} "
                          "registered={} os_result={} fix_call#={}",
                          p2p_count, epoll->events.size(), result, c);
            }
        }

        if (p2p_count > 0) {
            i = p2p_count;
            LOG_INFO(Lib_Net, "P2P epoll fix: generated {} events from vport queues", p2p_count);
        }
    }
...
int PS4_SYSV_ABI sceNetIoctl(int fd, u32 code, void* buf) {
    // Enhanced logging for P2P signaling ioctls
    if (code == 0x802450c8 && buf) {
        // Signaling ioctl: type code at buf+0x00, gate byte at buf+0x23
        u32 type_code = *reinterpret_cast<u32*>(buf);
        u8 gate_byte = *reinterpret_cast<u8*>(reinterpret_cast<u8*>(buf) + 0x23);
        const char* type_name = "other";
        if (type_code == 5)
            type_name = "ACTIVATE";
        else if (type_code == 6)
            type_name = "DEACTIVATE";
        LOG_INFO(Lib_Net, "sceNetIoctl: fd={} code={:#x} type={}({}) gate_byte={:#x}", fd, code,
                 type_name, type_code, gate_byte);
    } else if (code == 0x802450cb) {
        LOG_INFO(Lib_Net, "sceNetIoctl: fd={} code={:#x} TUNNEL_ESTABLISH", fd, code);
    } else if (code == 0x802450cc) {
        LOG_INFO(Lib_Net, "sceNetIoctl: fd={} code={:#x} TUNNEL_TEARDOWN", fd, code);
    } else {
        LOG_INFO(Lib_Net, "sceNetIoctl: fd={} code={:#x} buf={}", fd, code, buf);
    }

    // Delegate P2P signaling ioctls to KernelP2PSubsystem
    if (code == 0x802450c8 || code == 0x802450cb || code == 0x802450cc) {
        return Libraries::Net::KernelP2PSubsystem::Instance().HandleIoctl(fd, code, buf);
    }

    LOG_WARNING(Lib_Net, "sceNetIoctl: unhandled code={:#x}", code);
    return ORBIS_OK;
}
...
OrbisNetId PS4_SYSV_ABI sceNetSocket(const char* name, int family, int type, int protocol) {
    LOG_INFO(Lib_Net, "sceNetSocket: name={} family={} type={} proto={}", name ? name : "null",
